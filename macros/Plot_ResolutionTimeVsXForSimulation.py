...
outdir = myStyle.GetPlotsDir(outdir, "Resolution_Time_ForSimulation/")
...
list_htitles = [
    ...
    ["t1_res_vs_xy", "t1_res", "Time resolution [ps]"],
    ["t2_res_vs_xy", "t2_res", "Time resolution [ps]"],
...
sub_colors = [colors[1], kBlack, colors[5], colors[2]]
sub_widths = [2, 2, 2, 4]
legend_name = [
    "Single-channel, tracker delay correction",
    "t1, tracker delay correction",
    "t2, tracker delay correction",
...
for i,info_entry in enumerate(all_histoInfos):
    ...
    # Increase line width for the "timeDiffTracker_vs_xy" histogram
    if info_entry.inHistoName == "timeDiffTracker_vs_xy":
        hist.SetLineWidth(sub_widths[i] + 2)  # Increase line width
    else:
