...
parser.add_option('-x','--xlength', dest='xlength', default = 0.49, help="X axis range [-x, x]")
...
position_center = -0.25#mf.get_central_channel_position(inputfile, "x")
if("HPK_W9" in dataset):
    position_center = -0.30
box_position_center = 0.25
outdirnew = myStyle.GetPlotsDir(outdir, "Resolution_Pos_Compare/")
...
boxes = getStripBox(inputfile, ymax=ylength, shift=box_position_center, pitch=pitch/1000.)
...
line_binary_readout.SetLineWidth(2)
line_binary_readout.SetLineStyle(3)
...
if debugMode:
    outdir_q = myStyle.CreateFolder(outdirnew, "q_ResPosVsX0/")
...
# Create legend
legend = TLegend(pad_center-0.35, 1-pad_margin-0.285, pad_center+0.35, 1-pad_margin-0.095)
...
legend.AddEntry(histLaser, "Laser", "l")
legend.AddEntry(hist, "120 GeV protons", "l")
...
save_path ="%sPositionResVsX_%s"%(outdirSave, dataset)
