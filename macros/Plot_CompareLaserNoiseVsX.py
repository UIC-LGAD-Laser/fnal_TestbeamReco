...
if organized_mode: 
    ...
    inputfile = TFile("%s/Noise/NoiseVsX.root"%(outdir))
else: 
...
canvas = TCanvas("canvas", "Noise vs X", 1000, 800)

noise = inputfile.Get("Noise")
...
htemp.GetYaxis().SetRangeUser(0.1, ylength)
...
Noise_laser = inputfileLaser.Get("baselineRMS_vs_x")
laser_allHistos = [Noise_laser]
...
canvas.Update()
canvas.SaveAs(outdirSave+"Noise_vs_X_"+dataset+".pdf")
