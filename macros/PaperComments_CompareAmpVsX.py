...
# Define output file
output_path = "%sPaperCommentsCompare_AmplitudeVsX"%(outdirSave)
if (is_tight):
    output_path+= "_tight"
output_path+= ".root"
...
inputfileLaser = TFile("%s%sPlotAmplitudeVsX.root"%("/uscms/home/dshekar/nobackup/laser_analysis/TestbeamReco/output/",laser_dataset+"/"))
histRightCh = inputfileLaser.Get("amplitude_vs_x_channel02")
histRightCh.SetLineColor(colors[0])
histRightCh.SetLineStyle(1)
histRightCh.SetLineWidth(2)
histLeftCh = inputfileLaser.Get("amplitude_vs_x_channel01")
histLeftCh.SetLineColor(colors[1])
histLeftCh.SetLineStyle(1)
histLeftCh.SetLineWidth(2)
histRightCh.Draw("hist SAME")
histLeftCh.Draw("hist SAME")
legend.AddEntry(histRightCh, "Strip 1", "lp")
legend.AddEntry(histLeftCh, "Strip 2", "lp")

histRightCh_temp = histRightCh.Clone("laser_amplitude_vs_x_channel02")
histLeftCh_temp = histLeftCh.Clone("laser_amplitude_vs_x_channel01")
...
save_path = "%sPaperCommentsAmplitudeAllChannels_vs_x"%(outdirSave)
canvas.SaveAs("%s.pdf"%save_path)
inputfileLaser.Close()
histRightCh_temp.Write()
histLeftCh_temp.Write()
