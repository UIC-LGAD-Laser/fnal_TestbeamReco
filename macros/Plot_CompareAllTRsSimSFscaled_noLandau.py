...
allHistos = [tr, jitter_ftbf_scaled]
names = ["tr_vs_x","jitter_scaled_vs_x"]
colors_new = [colors[2], colors[0], colors[4]]
labels = ["Total time resolution (t)", "Scaled weighted jitter (j_{s})"]
...
for i,j in zip(laser_allHistos,["tr_vs_x","jitter_scaled_vs_x","landau_vs_x"]):
...
laser_all_histos_shifted_scaled = [tr_laser_unscaled, jitter_laser_scaled]
...
canvas.Update()
canvas.SaveAs(outdirSave+"All_TRsScaled_noLandau_"+dataset+".pdf")
